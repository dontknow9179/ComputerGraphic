...
// Others.
bool pauseAnimation = false;    // Freeze the cars if true.
...
void DrawOneCar(float bodyColor[3])
{
	...
	glPushMatrix();
	glTranslated(0, 0, CAR_HEIGHT * 7 / 8);
	glScaled(CAR_LENGTH / 1.5, CAR_WIDTH, CAR_HEIGHT / 4);
	glutSolidCube(1);
	glPopMatrix();
	...
	// Draw the four tyres.
	glPushMatrix();
	glTranslated(CAR_LENGTH / 4, CAR_WIDTH / 2, CAR_HEIGHT / 6);
	glRotated(90, 1, 0, 0);
	glutSolidTorus(CAR_HEIGHT / 8, CAR_HEIGHT / 6, 30, 60);
	glPopMatrix();
